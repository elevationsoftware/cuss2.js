...
/**
 * Device Types describe how applications handle the device and how to advise users<p>  *NON_APPLICABLE_DEVICE_TYPE:* Device does not fit in any category <br>  *PRINT:* Device can print documents<br>  *READ:* Device can read documents<br>  *MOTORIZED:* Device can move in and out a document (e.g. cards)<br>  *DIP:* User has to dip in and remove a document (e.g. cards or passports)<br>  *SWIPE:* User has to swipe the document or card<br>  *CONTACTLESS:* Device can read a document from a (usually short) distance<br>  *HANDHELD:* Device can scan bar codes and and needs to be handled manually<br>  *INSERT:* Documents have to be inserted into the device<br>  *DISPENSE:* Device can dispense a document to the user or another component<br>  *CAPTURE:* Device can retract documents or cards<br>  *CONVEYOR:* Device can move baggage back and forth<br>  *SCALE:* Device can weigh items<br>  *CAMERA:* Device can take pictures<br>  *FLATBED:* Device can scan documents<br>  *CHIP_AND_PIN:* Device is a Chip & PIN device / payment device<br>  *EXTERNAL:* Device is not contained in the CUSS device<br>  *BIOMETRICS:* Device is a biometrics device / interface <br>  *ASSISTIVE:* Device can assist users with disabilities<br>  *ILLUMINATION:* Device represents a light<br>  *DISPLAY:* Device represents a display (e.g. a touch screen) 
 */
export type DeviceTypes = 'NON_APPLICABLE_DEVICE_TYPE' | 'PRINT' | 'READ' | 'MOTORIZED' | 'DIP' | 'SWIPE' | 'CONTACTLESS' | 'INSERT' | 'DISPENSE' | 'CAPTURE' | 'CONVEYOR' | 'SCALE' | 'CHIP_AND_PIN' | 'EXTERNAL' | 'BIOMETRICS' | 'ASSISTIVE' | 'ILLUMINATION' | 'DISPLAY';

export const DeviceTypes = {
    NONAPPLICABLEDEVICETYPE: 'NON_APPLICABLE_DEVICE_TYPE' as DeviceTypes,
